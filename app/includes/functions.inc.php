...
function loginUser($conn, $erabiltzaile, $pasahitza) {
    ...
    var_dump($uidExists); // Agregar esta línea para verificar lo que devuelve uidExists
    ...
    var_dump($checkPas); // Agregar esta línea para verificar el resultado de password_verify

    if ($checkPas === false) {
        header("location: ../login.php?error=wronglogin");
        exit();
    } else if ($checkPas === true) {
        session_start();
        ...
        $_SESSION["erabiltzaileId"] = $uidExists["erabiltzaileId"];
